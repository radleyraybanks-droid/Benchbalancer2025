...
class BasketballUI {
    ...
    updateCourtWithScoring(state) {
        ...
        const courtContainer = this.elements.courtPlayers;
        if (!courtContainer) return;

        courtContainer.querySelectorAll('.player-position').forEach(element => {
            const playerName = element.dataset.player;
            if (!playerName) return;

            const points = scoring.playerPoints?.[playerName] || 0;
            const icon = element.querySelector('.player-icon');

            if (icon) {
                const existingBadge = icon.querySelector('.player-points');
                if (existingBadge) {
            ...
            if (points > 0 && icon) {
                ...
                pointsBadge.className = 'player-points';
                pointsBadge.textContent = `${points} pts`;
                icon.appendChild(pointsBadge);
            }

            const existingControls = element.querySelector('.player-scoring');
            if (existingControls) {
            ...
            scoringControls.className = 'player-scoring';
            scoringControls.innerHTML = `
                <button class="score-btn minus" onclick="basketballUI.updatePlayerScore('${jsName}', -1)">-</button>
                <button class="score-btn" onclick="basketballUI.updatePlayerScore('${jsName}', 1)">+</button>
            `;
    ...
    updateCourtMobile(state) {
        ...
        state.players.court.forEach((player, idx) => {
            const points = state.scoring?.playerPoints?.[player] || 0;
            ...
            courtHTML += `
                <div class="player-position ${posClass}" data-player="${safeName}">
                    <div class="player-badge">
                        <div class="player-name-badge">${displayName}</div>
                        <div class="player-score-badge">${scoreLabel}</div>
                        <div class="badge-scoring">
                            <button class="badge-score-btn minus" onclick="basketballUI.updatePlayerScore('${jsName}', -1)">-</button>
                            <button class="badge-score-btn" onclick="basketballUI.updatePlayerScore('${jsName}', 1)">+</button>
                        </div>
                    </div>
                </div>
            `;
    ...
    updateTimer(state) {
        ...
        // Period timer
        const periodMinutes = Math.floor(state.periodElapsed / 60);
        const periodSeconds = state.periodElapsed % 60;
    ...
    /**
     * Update court player display with positions and times
     */
    updateCourt(state) {
        const positions = ['pg', 'sg', 'sf', 'pf', 'c'];
        ...
        state.players.court.forEach((player, idx) => {
            const position = state.players.positions[player] || positions[idx].toUpperCase();
            const posClass = `pos-${positions[idx]}`;
            ...
            courtHTML += `
                <div class="player-position ${posClass}" data-player="${safeName}" data-role="${positions[idx]}">
                    <div class="player-icon" role="presentation">
                        ${nextOutBadge}
                        <span class="icon-base" aria-hidden="true">🏀</span>
                    ...
                    <div class="player-info">
                        ...
                        <div class="player-time-display">
                            <div class="time-row">
                                <span class="time-label">Play</span>
                                <span class="time-value">${this.formatTime(minutes)}</span>
                            </div>
                            <div class="time-row">
                                <span class="time-label">Bench</span>
                                <span class="time-value">${this.formatTime(benchMinutes)}</span>
                            </div>
                            <div class="time-row">
                                <span class="time-label">Stint</span>
                                <span class="time-value">${this.formatTime(currentStint)}</span>
                            </div>
                        </div>
                    </div>
                    <div class="position-badge">${safePosition}</div>
                </div>
            `;
        });
