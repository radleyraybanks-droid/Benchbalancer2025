// --- START OF FILE timer-and-gameplay.js ---

// ... (applyMissedTime, handleVisibilityChange, gameLoop functions remain the same as your last version) ...
...
function handleVisibilityChange() {
    ...
    if (document.hidden) {
        ...
        if (wasRunningWhenHidden && lastVisibleTimestamp && !isGameOver && !isSetupScreenVisible && !isHalftimeScreenActive) {
            ...
            // Defensive check: If elapsed time is unreasonably large (>1 hour), likely a stale timestamp
            if (elapsedWhileHidden > 3600) {
                debugLog(`WARNING: Elapsed time (${formatTime(elapsedWhileHidden)}) seems excessive. Likely stale timestamp. Skipping catch-up.`);
...
function gameLoop() {
    ...
    // Add to gameLoop function after updateDisplay():
    if (typeof recordVarianceHistory !== 'undefined') {
        recordVarianceHistory();
    }
    
    // Add variance display update
    if (typeof calculateCurrentVariance !== 'undefined') {
        const balanceTracker = document.getElementById('balanceTracker');
        if (balanceTracker && balanceTracker.updateDisplay) {
            balanceTracker.updateDisplay();
        }
    }
}
...
function startTimer(isResume = false) {
    ...
    if (typeof resetVarianceTracking !== 'undefined' && currentGameSeconds === 0) {
    ...
    isRunning = true;
    // DO NOT set wasRunningWhenHidden or lastVisibleTimestamp here
    // These should only be set by visibility change handlers when page actually becomes hidden
